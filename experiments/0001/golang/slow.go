...
func Slow(conf Config) {
	// Create a backup of the original file, so that we can restore it after slaying the mutants.
	copyFile(conf.OriginalFilePath, conf.BackupFile)

	mutants, err := os.ReadDir(conf.MutantsDir)
	if err != nil {
	...
	for _, m := range mutants {
		...
		mutatedFilePath := filepath.Join(conf.MutantsDir, m.Name(), "src", conf.OriginalFileName)
		copyFile(mutatedFilePath, conf.OriginalFilePath)
		slain := slayMutant()
		if slain {
			...
			err := os.RemoveAll(filepath.Join(conf.MutantsDir, m.Name()))
			if err != nil {
	...
	cleanUp(conf.BackupFile, conf.OriginalFilePath)
}
...
func cleanUp(backupFile, origFilePath string) {
	copyFile(backupFile, origFilePath)
